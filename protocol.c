...
/* 
    ...
    mcu_dp_bool_update(DPID_SWITCH_ALARM_SOUND,当前报警声开关,子设备ID,子设备ID长度); //BOOL型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_switch_alarm_sound_handle
功能描述 : 针对DPID_SWITCH_ALARM_SOUND的处理函数
输入参数 : value:数据源数据
        : length:数据长度
        : sub_id_buf:子设备id
...
*****************************************************************************/
static unsigned char dp_download_switch_alarm_sound_handle(const unsigned char value[], unsigned short length, unsigned char *sub_id_buf, unsigned char sub_id_len)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char switch_alarm_sound;
    
    switch_alarm_sound = mcu_get_dp_download_bool(value,length);
    if(switch_alarm_sound == 0) {
        //开关关
        /*****************************************************************************
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SWITCH_ALARM_SOUND,switch_alarm_sound, sub_id_buf, sub_id_len);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length,unsigned char* sub_id_buf,unsigned char sub_id_len)
{
    ...
    ****************************************************************/
    unsigned char ret;
    switch(dpid) {
        case DPID_SWITCH_ALARM_SOUND:
            //报警声开关处理函数
            ret = dp_download_switch_alarm_sound_handle(value,length,sub_id_buf,sub_id_len);
        break;
